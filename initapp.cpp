...
using namespace std;
//creation gestion de stock tables
void stockManagementTables()
{
    //projects
    Db *projectsDb=new Db("articles.rov");

    if(projectsDb->getTotalRows()<=0) {
        string cols("id_rov_article reference_rov_article name_rov_article description_rov_article price_rov_total stock_rov_");
        projectsDb->addData(cols);
    }

}
//creation gestion code tables
void codeManagementTables()
{
    //projects
    Db *projectsDb=new Db("codes.rov");

    if(projectsDb->getTotalRows()<=0) {
        string cols("id_rov_project name_rov_project language_rov_");
        projectsDb->addData(cols);
    }
